 ...
 * example clicking on a submit button, which will call <code>ConcurrentWorker.post()</code>
 ...
 * for example clicking on a cancel button, which will call <code>ConcurrentWorker.post()</code>
 ...
 * the first invocation of <code>ConcurrentWorker.post()</code> (the one that submitted the
 * task to the server) does not return until <i>event2</i> and hence the second invocation
 * of <code>ConcurrentWorker.post()</code> is finished. <br />
 * Therefore, this class will execute tasks concurrently, but invocations of
 * <code>ConcurrentWorker.post()</code> are executed one after the other.
 * This ensures that the code after the second invocation of
 * <code>ConcurrentWorker.post()</code> is always executed before the code after the first
 * invocation of <code>ConcurrentWorker.post()</code> (this is not guaranteed when
